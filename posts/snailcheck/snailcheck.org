...
The problem is that we are always evaluating the recursive calls in the =cons3= branch, so we recurse infinitely.

[fn:mechanical] That means we can automate the generation of enumerations of algebraic data types!

*** Solution
Imagine you are =tree_a=. This is roughly the behavior we want:

- I'm giving you the first tree in the enumeration. That's =Leaf=.
...
We can't do this with strict evaluation, so we need to add a bit of laziness. We will rewrite =interleave= so that its arguments are both lazy (=delayed=).

*** IDEA Order (currently) matters
Remember when I said we needed to grab the base case(s) before the recursive case(s)? With the current code, that's true.

It's also problematic. It's annoying to remember and results in an unhelpful error message.[fn:order-error] What if we want to generate a mutually recursive ADT?

TODO: Mention that this problem also occurs in PearlCheck.

TODO: Are there any mutually recursive data types that don't result in their only possible values being infinite cycles?
...
I'm thinking proper detection would require some sort of static analysis, maybe could be done with a [[https://ocaml.org/docs/metaprogramming][PPX]].
